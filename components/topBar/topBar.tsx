...
export default function TopBar() {
    ...
    const getResiduos = async () => {
        try {
            const res: AxiosResponse<ResiduoResponse[]> = await axiosInstance.get("/api/Residuos?Filters=IdUsuario%3D%3D" + usuario?.idUsuario);
                ...
                <Dropdown menu={{ items }} trigger={['click']}>
                    <button className="flex items-center justify-between gap-2 p-2 px-4 rounded-lg border-0 hover:bg-brown/10  ">
                        <div className='flex items-center gap-2'>
                            ...
                            <span className='font-extrabold'>{usuario?.username}</span>
